...
public class ChangelogGenerator(ChangelogSettings config)
{
    ...
    {
        // todo - what if and additional issue found?

        var remainingCommitsLookup = new CommitMessages();
        remainingCommitsLookup.TryAddRange(commits);
        ...
        foreach (var handledChange in lastRunData.HandledChanges)
        {
            ...
            var commitIssues = commitMetadata.FooterKeyValues["issues"].ToList();
            commitIssues.AddRange(commitMetadata.FooterKeyValues["issue"]);
            commitIssues.AddRange(commitMetadata.FooterKeyValues["ref"]);
            commitIssues.AddRange(commitMetadata.FooterKeyValues["refs"]);
        ...
        {
            var commitMetadata = commit.MessageMetadata;
            var issues = commitMetadata.FooterKeyValues["issues"].ToList();
            issues.AddRange(commitMetadata.FooterKeyValues["issue"]);
            issues.AddRange(commitMetadata.FooterKeyValues["ref"]);
            issues.AddRange(commitMetadata.FooterKeyValues["refs"]);
    ...
    {
        var changeEntries = new ChangeLogEntryMessages();
        foreach (var commit in commits)
        {
            var messageMetadata = commit.MessageMetadata;
            if (!changeEntries.TryGet((messageMetadata.ChangeTypeText, messageMetadata.ChangeDescription), out var logEntry))
            {
                logEntry = new ChangeLogEntry(messageMetadata);
                logEntry.AddIssues(messageMetadata.FooterKeyValues["issues"]);
                logEntry.AddCommitId(commit.CommitId.ShortSha);
                changeEntries.Add(logEntry);
                continue;
            }
            else
            {
